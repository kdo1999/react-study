...
const UseMemoSample = () => {
    ...
    const numberOfCharacters1 = items.reduce((sub, item) => {
        console.log(`현재 item: ${item}, item의 길이: ${item.length}, 현재까지의 합: ${sub}`);
        return sub + item.length;
    }, 0)

    // numberOfCharacters2는 userMemo를 사용해, items가 업데이트되는 시점에 items.reduce를 실행해서 결과를 얻는다.
